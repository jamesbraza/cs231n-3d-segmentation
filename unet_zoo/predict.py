...
from data.loaders import BraTS2020MRISlicesDataset
...
BEST_MODEL = CHECKPOINTS_FOLDER / "best_checkpoint_2d.pytorch"
THRESHOLD = 0.45
DEVICE = infer_device()
...
def make_summary_plots(
    ...
    for images, targets in DataLoader(BraTS2020MRISlicesDataset(scans_ds=test_ds)):
        with torch.no_grad():
            preds = model(images)
...
def main(prune_weight) -> None:
    ...
    dices = []
    inf_times = []
    for images, targets in tqdm(DataLoader(BraTS2020MRISlicesDataset(scans_ds=test_ds)),
            desc="compiling ious",
    ):
        with torch.no_grad():
            start = time.perf_counter()
            preds = model(images.squeeze(dim=-3)).unsqueeze(dim=-3) >= THRESHOLD
            stop = time.perf_counter()
        inf_times.append(stop - start)
        ious.append(MeanIoU()(input=preds, target=targets) * 100)
        ret_dice = DiceCoefficient()(input=preds, target=targets) * 100 
